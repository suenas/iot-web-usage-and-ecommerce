...
In this section, we give an overview of the updated IFMLModels for the Madison Island website based on the transformations discussed in the last chapter describing how their IFMLDiagrams and corresponding website frontend screens would look like.
...
After transforming its IFMLModel component (\ref{header-updates}), the shared header section on the website reveals new information through a new widget aligned on the right side of the area for advertising the actions the user made in the retail store with the corresponding earned reward points.
...
Regardless of the DisplayMode property set for each Category Page on the eCommerce platform, the updated web model presents a vertical section on the right sidebar which maps the recently browsed URLs recorded in the same user session which have been identified during the model transformation phase (\ref{category-page-updates}).
...
In \ref{shopping-cart-updates} we outlined how the visual aspect of the shopping cart sidebar would mutate through an \ref{IFMLModel} transformation producing a new form element within the sidebar responsible for controlling the application of the accumulated reward points available to the customer until that point. Those reward points, generated through actions recorded as part of the real usage data profiling, convert into a discount amount for the current quote that the customer is free to use at any time. As per shown by the attached action in the IFMLDiagram, the customer is redirected to the shopping cart page after the discount is applied.

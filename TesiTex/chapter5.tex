...
For example, we know that \textbf{Blazers},\textbf{Tees, Knits and Polos} and \textbf{Shoes} categories are the \textit{sessionRegion} elements where the user spent more time within the store sorting them by \textit{maxSecondsInRegion} and \textit{maxProximity}. In the case of the Blazers category for example, we would have this data in the \textit{RealUsageData:ProximityData} node:
...
Using the above information, we can now potentially transform the \textit{subViewComponentParts} and the \textit{Parameter} nodes of the first HighlightedCategoriesCarousel IFMLWindow from this :  

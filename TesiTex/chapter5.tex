...
For what it concerns the possible enhancements on the product page of the Madison Island website we focused on the Related Products widget which is shown just below the Add To Cart section. Assuming an initial setup where the list of the products related to the currently displayed one has no specific logic attached, we can proceed extending the \textit{viewElements} node of the IFMLModel taking into consideration the browsing pattern of the users which have been identified examining the RealUsageData instances entries.

For example, we could have discovered that the product #413 is frequently browsed with the Product #404 in the same sessions examining the values set in the /textit{viewContainer}, /textit{viewComponent} and /textit{parameterBindingGroup} properties for the nodes belonging to the /textit{RealUsageData:WebData} dataset. Consequently, we can take advantage of this information to push a product rather than another on the Related Products section offering a more tailored and customized experience to the user.

With this know we can proceed 

...
As the customer walks around the shop, proximity events must be recorded from its Madison Island mobile app containing data about each iBeacon detected along the way and its relative distance when the match has occurred indicating which aisles customers visited and which not.

Considering the above, the following would be a suitable Estimote beacons allocation for the store :
...
Depending on the business use case the Madison Island app can wether send an event to the listening server whenever a new beacon identifier is first detected along with the distance at that time or submit a single event with a full list of beacons detected during the staying along with their minimum/maximum proximities.Due to the behavioral profiling nature of this activity, we assume the app behaves like the latter and performs a data push to the web server when the shoppers leave the store.

...
This map describes a potential iBeacon allocation for a physical Madison Island retail store which would mirror the catalog hierarchy in its web form. 

\vspace{0.5cm}
\begin{figure}[H]
  \centering
    \includegraphics[width=16cm]{images/madison/retail-map.jpg}
  \caption{Madison Island retail store with iBeacon allocation}
  \label{fig:ifml2}
\end{figure}
\vspace{0.5cm}

As the customer walks around the shop, proximity events would be recorded from its Madison Island mobile app containing data about each iBeacon detected along the way and their distance when the match has occurred.

Depending on the business use case the app can wether send an event to the listening server whenever a new beacon identifier is first detected along with the distance at that time or submit an event periodically every 10 minutes with a full list of beacons seen during that period along with their minimum/maximum proximities.

Considering the behavioral profiling nature of this activity, we assume the app behaves like the latter and performs a data push to the web server at regular intervals.

The gathered data would eventually have this form :

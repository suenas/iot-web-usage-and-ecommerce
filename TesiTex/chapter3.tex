...
In the context of this thesis work, we will focus on a basic scenario where the customer proximity recognition happening in the real world does not directly trigger any immediate action to grab customer attention but it limits itself to silently track the event sending a communication to a listening web server.

The IoT device of choice to illustrate this example would be the Beacon, specifically the Estimote Beacons which are using Apple iBeacon tehcnology and are compatible with iBeacon-enabled Apple products and applications.

\vspace{0.5cm}
\begin{figure}[H]
  \centering
    \includegraphics[width=16cm]{images/ibeacon.png}
  \caption{An Estimote iBeacon device}
  \label{fig:ifml2}
\end{figure}
\vspace{0.5cm}


As per previously described in \ref{beacons} the beacon can simply be seen as a lighthouse broadcasting information leveraging Low Energy Bluetooth connection. In the case of iBeacons the information sent to listening mobile devices would contain:

The Universal Unique ID (UUID) is globally unique. Example: de2b45ae-ed98-11e4-3432-78616d6f6f6d
The Major ID uniquely identifies our customer’s system: e.g. 51314
The Minor ID reports the exact location or object (in our case, the spot): e.g. 23369
